...
public sealed class RewardServiceTests
{
    ...
    private sealed class FakeRewardRepository : IRewardRepository
    {
        ...
        public Task<RewardGrantResult> GrantAsync(RewardGrantRequest request, CancellationToken cancellationToken=default)
        {
            ...
            var e=new RewardEvent{Id=1,UserId=request.UserId,RuleCode=request.RuleCode,RuleVersion=1,Reason="Vote cast",SourceType=request.SourceType,SourceReference=request.SourceReference,SourceKey=key,Value=25,CreatedAt=request.OccurredAtUtc};
            _events[key]=e; return Task.FromResult(new RewardGrantResult(e,25,false));
        }
        public Task<RewardEvent> ReverseAsync(long eventId,long actorUserId,string reason,string key,CancellationToken token=default)=>throw new NotImplementedException();
        public Task<RewardEvent> AdjustAsync(long userId,int value,long actorUserId,string reason,string key,CancellationToken token=default)=>throw new NotImplementedException();
        public Task<IEnumerable<RewardEvent>> GetEventsAsync(long? userId,int count,CancellationToken token=default)=>throw new NotImplementedException();
