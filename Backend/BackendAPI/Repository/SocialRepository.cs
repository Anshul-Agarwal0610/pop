...
public sealed class SocialRepository(DapperContext context) : ISocialRepository
{
    ...
    public async Task BlockAsync(long actorId,long targetId)
    {
        ...
        await db.ExecuteAsync("UPDATE LiveSessionInvitations SET State='Cancelled',ConsumedAt=SYSUTCDATETIME() WHERE State='Pending' AND ((InviterUserId=@Actor AND IntendedUserId=@Target) OR (InviterUserId=@Target AND IntendedUserId=@Actor))",new{Actor=actorId,Target=targetId},tx);
        await db.ExecuteAsync("UPDATE LiveSessionParticipants SET NotificationsEnabled=0 WHERE UserId IN(@Actor,@Target) AND SessionId IN(SELECT SessionId FROM LiveSessionParticipants WHERE UserId IN(@Actor,@Target) GROUP BY SessionId HAVING COUNT(DISTINCT UserId)=2)",new{Actor=actorId,Target=targetId},tx);
        tx.Commit();
    }
