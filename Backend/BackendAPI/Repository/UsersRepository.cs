    ...
    public class UsersRepository : IUsersRepository
    {
        ...
        public async Task<VoteRewardResult> ApplyVoteRewardAsync(long userId, int xpToAdd, DateTime utcNow)
        {
            ...
            if (consent == "granted")
            {
                ...
                var previousLevel = user.Xp / 1000 + 1; var currentLevel = updated.Xp / 1000 + 1;
        ...
        public async Task<AnalyticsPrivacyPreference> UpdateAnalyticsPrivacyAsync(long userId, string consent)
        {
            ...
            if (normalized is not ("unknown" or "granted" or "denied")) throw new ArgumentException("Consent must be unknown, granted, or denied.");
            using var conn = _context.CreateConnection();
